...
class ContainsOperatorTest extends BaseUnitTest {
  ...
  private static Stream<Arguments> validArgumentProvider() {
    return Stream.of(
        Arguments.of(FilterOperator.CONTAINS, 1, 0),
        Arguments.of(FilterOperator.NOT_CONTAINS, 0, 1),
        Arguments.of(FilterOperator.STARTS_WITH, 1, 0),
        Arguments.of(FilterOperator.ENDS_WITH, 1, 0)
    );
  }
  ...
  void apply_OK(FilterOperator filterOperator, Integer mustQuerySize, Integer mustNotQuerySize) {
    ...
    // WHEN
    assertDoesNotThrow(() -> operator.apply(expectedMustQuery, expectedMustNotQuery, filter));
